...
public:
    ...
    void click_attack()
    {
        for (auto &e : items)
        {
            if (e.isClickable)
            {
                if (CheckCollisionPointRec(GetMousePosition(), {e.x, e.y, e.x + e.width, e.y + e.height}))
                {
                    DrawText("Collision", 50, 100, 23, RED);
                    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT))
                    {
                        // DrawText("Clicked", 50, 100, 23, RED);
...
int main(void)
{
    ...
    ImageResize(&KPImage, 2500, 1000);
    auto KPTexture = LoadTextureFromImage(KPImage);
    // auto KPTexture = LoadTexture("resources/images/characters/kalupandey.png");
 ...
 nueu Ps jif{ kl5, @ cS6f]a/ !&$$ df, pgn] g'jfsf]6nfO{ km]l/ cfqmd0f u/] / lht]/ cfkm\gf] uf]/vf /fHosf] l;dfgf lj:tf/ u/].)";
    ...
    while (!exitWindow && !WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        case STRATEGY:
        {
            ...
            ClearBackground(WHITE);
            DrawTexture(skyTexture, 0, 0, WHITE);
            DrawTexture(PNSTexture, -800, 220, WHITE);
            DrawTexture(KPTexture, -700, 50, WHITE);
